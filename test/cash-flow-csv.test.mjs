...
test('maps signed imported amounts to cash flows and reports invalid rows', () => {
  const result = mapImportedRows([{ When: '01/04/2026', Value: '100' }, { When: '02/04/2026', Value: '-25,50' }, { When: 'no date', Value: '2' }], { dateColumn: 'When', amountColumn: 'Value', dateFormat: 'dmy' });
  assert.deepEqual(result.flows, [{ date: '2026-04-01', type: 'inflow', amount: 100 }, { date: '2026-04-02', type: 'outflow', amount: 25.5 }]);
  assert.deepEqual(result.invalidRows, [4]);
});
