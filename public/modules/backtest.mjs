...
function saleForLot(lot, units, price, exemption, year, applyExemption) {
  const gross = units * price;
  const tob = gross * TOB_RATE;
  const gain = Math.max(0, (price - (lot.taxPurchasePrice ?? lot.purchasePrice)) * units);
  if (year < CGT_EXEMPTION_START_YEAR) return { gross, tob, cgt: 0, exoneratedCgt: gain * CGT_RATE, net: gross - tob, gain };
  const exemptedGain = applyExemption ? exemptGain(exemption, gain, year) : 0;
  const cgt = (gain - exemptedGain) * CGT_RATE;
  return { gross, tob, cgt, exoneratedCgt: exemptedGain * CGT_RATE, net: gross - tob - cgt, gain };
}

function liquidation(lots, prices, price, exemption, year) {
  return lots.reduce((total, lot) => {
    establishTaxPurchasePrice(lot, prices, year);
    const sale = saleForLot(lot, lot.units, price, exemption, year, true);
    return { gross: total.gross + sale.gross, tob: total.tob + sale.tob, cgt: total.cgt + sale.cgt, net: total.net + sale.net };
  }, { gross: 0, tob: 0, cgt: 0, net: 0 });
}

function unitsForNetOutflow(lot, maxUnits, price, targetNet, exemption, year) {
  let lower = 0;
  let upper = maxUnits;
  for (let iteration = 0; iteration < 60; iteration += 1) {
    const units = (lower + upper) / 2;
    const simulatedExemption = { ...exemption };
    if (saleForLot(lot, units, price, simulatedExemption, year, true).net < targetNet) lower = units;
    else upper = units;
  }
  return upper;
}
...
export function runBacktest(flows, prices, valuationDate, { applyCapitalGainsExemption = false, buyWholeSharesOnly = false, unpaidAccruedInterest = 0, brokerTransactionFee = 0 } = {}) {
  ...
  for (const flow of [...flows].sort((left, right) => left.date.localeCompare(right.date))) {
    ...
    for (const lot of lots) {
      ...
      establishTaxPurchasePrice(lot, prices, saleYear);
      const fullSale = saleForLot(lot, lot.units, quote.price, { ...exemption }, Number(flow.date.slice(0, 4)), true);
      const fractionalUnits = fullSale.net <= remainingNet ? lot.units : unitsForNetOutflow(lot, lot.units, quote.price, remainingNet, exemption, Number(flow.date.slice(0, 4)));
      const units = buyWholeSharesOnly ? Math.ceil(fractionalUnits) : fractionalUnits;
      const sale = saleForLot(lot, units, quote.price, exemption, Number(flow.date.slice(0, 4)), true);
      lot.units -= units;
