...
A local, browser-based simulator for dated EUR cash flows hypothetically invested in Amundi Smart Overnight Return UCITS ETF Acc (CSH2, ISIN `LU1190417599`). It models 0.12% TOB on both purchases and sales, an optional fixed broker fee per executed CSH2 trade, FIFO withdrawals expressed as net cash received, and assumes a 10% tax on each positive FIFO gain when you sell.
