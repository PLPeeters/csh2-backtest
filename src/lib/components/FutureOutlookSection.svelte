...
{#if setupTab === 'historical'}
  <section class="future-empty min-w-0 bg-[#fbfdfc] px-4.5 py-5.5 border border-dashed border-line rounded-[5px]" aria-labelledby="future-outlook-heading">
    <p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Forward guidance</p>
...
{:else}
  <section class="future-outlook min-w-0" aria-labelledby="future-outlook-heading">
    <div class="future-intro mt-0.5 mb-[17px] mx-0.5">
      <p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Forward guidance</p>
    ...
    <section class="suggested-next-move mt-0 mb-4 [border-left-width:3px] [border-left-style:solid] [border-left-color:var(--v2-green-dark)] bg-[#f1f7f3] mx-0 px-[15px] py-3" aria-labelledby="suggested-next-move-heading">
      <p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Suggested next move</p>
    ...
    {#if view && projection}
      <section class="panel projection-panel [background:var(--surface)] p-5.5 border border-solid [border-color:var(--line)] rounded-[8px] [@media(width<=760px)]:p-5 [@media(760px<width<=1050px)]:p-4.5" aria-labelledby="projection-heading">
        <div class="section-title flex items-center justify-between pb-5 gap-5 [@media(width<=760px)]:items-start [@media(width<=760px)]:flex-col [@media(width<=760px)]:gap-3">
          <div><p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Selected scenario</p><h3 id="projection-heading">Projected performance</h3></div>
          <div class="return-mode-picker flex overflow-x-auto bg-white border border-solid border-[#cbd7d0] rounded-[7px] [@media(width<=760px)]:w-full" role="group" aria-label="Projection view">
    ...
    {#if result?.fidelityPremiumAssessments.length}
      <section class="panel fidelity-outlook [background:var(--surface)] p-5.5 border border-solid [border-color:var(--line)] rounded-[8px] [@media(width<=760px)]:p-5 [@media(760px<width<=1050px)]:p-4.5" aria-labelledby="fidelity-outlook-heading">
        <div class="section-title flex items-center justify-between pb-5 gap-5 [@media(width<=760px)]:items-start [@media(width<=760px)]:flex-col [@media(width<=760px)]:gap-3"><div><p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Transfer timing</p><h3 id="fidelity-outlook-heading">Fidelity premium timing</h3></div></div>
        <div class="table-wrap w-full max-w-full min-w-0 overflow-x-auto overscroll-x-contain"><table class="fidelity-timing-table mb-0 text-[0.8rem]"><thead><tr><th>Base amount</th><th>Earned</th><th>Payout</th><th>Until payout</th><th>Next full year</th><th>Recommendation</th></tr></thead><tbody>
          {#each result.fidelityPremiumAssessments as assessment (assessment.id)}
            {@const nextYear = nextYearComparison(assessment)}
            <tr><td><span class="fidelity-mobile-label hidden [@media(width<=760px)]:block [@media(width<=760px)]:text-[#60756d] [@media(width<=760px)]:text-[0.68rem] [@media(width<=760px)]:font-bold [@media(width<=760px)]:tracking-[0.08em] [@media(width<=760px)]:uppercase" aria-hidden="true">Base amount</span>{euro.format(assessment.baseAmount)}</td><td><span class="fidelity-mobile-label hidden [@media(width<=760px)]:block [@media(width<=760px)]:text-[#60756d] [@media(width<=760px)]:text-[0.68rem] [@media(width<=760px)]:font-bold [@media(width<=760px)]:tracking-[0.08em] [@media(width<=760px)]:uppercase" aria-hidden="true">Earned</span>{date.format(new Date(`${assessment.earnedDate}T00:00:00Z`))}</td><td><span class="fidelity-mobile-label hidden [@media(width<=760px)]:block [@media(width<=760px)]:text-[#60756d] [@media(width<=760px)]:text-[0.68rem] [@media(width<=760px)]:font-bold [@media(width<=760px)]:tracking-[0.08em] [@media(width<=760px)]:uppercase" aria-hidden="true">Payout</span>{euro.format(assessment.finalPayoutAmount)}</td><td><span class="fidelity-mobile-label hidden [@media(width<=760px)]:block [@media(width<=760px)]:text-[#60756d] [@media(width<=760px)]:text-[0.68rem] [@media(width<=760px)]:font-bold [@media(width<=760px)]:tracking-[0.08em] [@media(width<=760px)]:uppercase" aria-hidden="true">Until payout</span>{assessment.currentPeriodPreferred === 'move now' ? `CSH2 +${euro.format(Math.abs(assessment.currentPeriodDifference))}` : assessment.currentPeriodPreferred === 'move to best account' ? 'Best savings account' : assessment.currentPeriodPreferred === 'wait' ? `Current account +${euro.format(Math.abs(assessment.currentPeriodDifference))}` : 'Effectively equal'}</td><td><span class="fidelity-mobile-label hidden [@media(width<=760px)]:block [@media(width<=760px)]:text-[#60756d] [@media(width<=760px)]:text-[0.68rem] [@media(width<=760px)]:font-bold [@media(width<=760px)]:tracking-[0.08em] [@media(width<=760px)]:uppercase" aria-hidden="true">Next full year</span>{nextYear.primary}{#if nextYear.otherAlternative}<span class="next-year-alternative">{nextYear.otherAlternative}</span>{/if}</td><td class="timing-recommendation"><span class="fidelity-mobile-label hidden [@media(width<=760px)]:block [@media(width<=760px)]:text-[#60756d] [@media(width<=760px)]:text-[0.68rem] [@media(width<=760px)]:font-bold [@media(width<=760px)]:tracking-[0.08em] [@media(width<=760px)]:uppercase" aria-hidden="true">Recommendation</span>{recommendation(assessment)}</td></tr>
          {/each}
        </tbody></table></div>
    ...
    {:else if view}
      <p class="future-note text-muted text-[0.76rem] leading-[1.5] mx-0.5 my-3">No unpaid fidelity premiums are entered, so there is no transfer timing recommendation to show.</p>
    {:else}
      <p class="future-note text-muted text-[0.76rem] leading-[1.5] mx-0.5 my-3">Calculate a current-account backtest to see projected performance and transfer timing.</p>
    {/if}
  </section>
{/if}
