...
<section class="benchmark-history-section" aria-labelledby="benchmark-heading">
  <div class="benchmark-savings-control">
    <label class="account-interest-rate benchmark-savings-input">Total savings amount (€)<input type="number" min="0.01" step="0.01" placeholder="Default: 10000" value={controller.settings.totalSavingsAmount} oninput={(event) => controller.setTotalSavingsAmount(event.currentTarget.value)} /></label>
  ...
  <section class="panel chart-panel benchmark-chart-panel">
    ...
    {#if controller.benchmarkAfterTax}<p class="chart-explanation tax-explanation">CSH2 includes buy and sell TOB plus {benchmarkUsesReyndersTax ? '30% Reynders Tax' : '10% CGT from 2026'}{#if appliesCgtExemption}, applying the annual CGT exemption to €{totalSavingsAmount.toLocaleString('nl-BE')}{:else if !benchmarkUsesReyndersTax}, ignoring the annual CGT exemption{/if}. The euro overnight benchmark is unchanged.</p>{/if}
    <div class="chart-update-container">
      ...
      {#if controller.benchmarkStatus.kind === 'loading' && selectedSeries}<div class="chart-update-overlay" role="status">Updating annualized returns…</div>{/if}
    </div>
  </section>
</section>
