...
<section class="input-section assumptions" aria-labelledby="assumptions-heading">
  ...
  <div class="setting-grid">
    ...
    <div class="setting-card"><label class="tax-exemption"><input type="checkbox" checked={controller.settings.applyReyndersTax} onchange={(event) => controller.updateSetting('applyReyndersTax', event.currentTarget.checked)} />Apply Reynders Tax instead of CGT</label><p class="setting-description">Applies 30% to all positive gains instead of CGT.</p></div>
  </div>
</section>
