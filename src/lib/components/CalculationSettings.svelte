...
<section class="input-section assumptions" aria-labelledby="assumptions-heading">
  ...
  <div class="setting-grid">
    ...
    <div class="setting-card"><label class="tax-exemption"><input type="checkbox" disabled={controller.settings.applyReyndersTax} checked={controller.settings.applyCapitalGainsExemption} onchange={(event) => controller.setCapitalGainsExemption(event.currentTarget.checked)} />Apply the annual capital-gains exemption</label><p class="setting-description">Applies a €10.000 annual exemption and eligible carry-forward up-to €15.000. The total savings amount also applies it to the estimates and annualized-return chart.</p></div>
  </div>
</section>
