...
<section class="benchmark-section mb-8" aria-labelledby="holding-period-heading">
  <div class="section-title current-rate-heading items-center flex justify-between pb-5 gap-5 [@media(width<=760px)]:items-start [@media(width<=760px)]:flex-col [@media(width<=760px)]:gap-3">
  <div><p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Current-rate estimate</p><h2 id="holding-period-heading">Minimum holding periods</h2>{#if holdingPeriods}<p class="current-rate-summary flex items-baseline flex-wrap gap-y-1.5 gap-x-4.5 mt-[9px] mb-0 text-[#5b7066] text-[0.84rem] mx-0" aria-label="Current rates used"><span>Current €STR <strong>{shownOvernightRate === undefined ? '—' : `${percent(shownOvernightRate)}%`}</strong></span><span class="estimated-rate inline-flex items-center gap-[5px]">Estimated CSH2 <strong class="estimated-rate-value inline-flex gap-[0.25em]"><span class="estimated-rate-point">{shownCsh2Rate === undefined ? '—' : `${percent(shownCsh2Rate)}%`}</span>{#if shownCsh2Error !== undefined && shownCsh2Error > 0}<span class="estimated-rate-error">±{percent(shownCsh2Error)} pp</span>{/if}</strong><span class="methodology-trigger relative inline-flex"><button type="button" class="methodology-info inline-grid w-5 min-h-5 place-items-center bg-white text-accent-dark [font-family:Georgia,_serif] text-[0.76rem] font-bold leading-[1] p-0 border border-solid border-[#6e857a] rounded-[50%]" aria-label="How estimated CSH2 is calculated" aria-haspopup="dialog" onclick={onOpenMethodology}>i</button><span class="methodology-tooltip absolute z-7 bottom-[calc(100%_+_7px)] left-[50%] w-max bg-[#17241f] text-white text-[0.72rem] leading-[1.2] opacity-0 pointer-events-none px-2 py-1.5 rounded-[5px]" role="tooltip">Click for methodology</span></span></span>{#if estimatedAfterTaxRate !== undefined}<br><span class="estimated-rate inline-flex items-center gap-[5px]">Post-tax estimated CSH2 rate <strong class="estimated-rate-value inline-flex gap-[0.25em]"><span class="estimated-rate-point">{shownAfterTaxRate === undefined ? '—' : `${percent(shownAfterTaxRate)}%`}</span>{#if shownAfterTaxError !== undefined && shownAfterTaxError > 0}<span class="estimated-rate-error">±{percent(shownAfterTaxError)} pp</span>{/if}</strong></span>{/if}{#if accountRateIsEntered && accountRateIsValid}<span>Best savings account <strong>{shownSavingsRate === undefined ? '—' : `${percent(shownSavingsRate)}%`}</strong></span>{/if}</p>{#if controller.settings.returnMode === 'real'}<p class="inflation-assumption mt-[7px] mb-0 text-[#617169] text-[0.78rem] leading-[1.45] mx-0">{#if annualInflation !== undefined && inflationObservation}Converted with {percent(annualInflation)}% observed Belgian inflation through {inflationObservation.lowerMonth}.{:else}Inflation-adjusted annual rates are unavailable because a full 12-month CPI comparison is not published.{/if} Holding periods and recommendation ordering continue to use the underlying cash amounts.</p>{/if}{/if}</div>
  </div>
  {#if holdingPeriods}
    <div class="chart-update-container relative"><div class="holding-period-summary mt-4.5 bg-white p-4.5 border border-solid border-[#d8e1db] rounded-[8px] [@media(width<=760px)]:p-3.5"><HoldingPeriodChart milestones={holdingPeriodMilestones} maximumDays={currentRateEvolution?.maximumProjectionDays ?? projectionHorizonDays} /></div>{#if controller.benchmarkStatus.kind === 'loading'}<div class="chart-update-overlay absolute z-2 grid place-items-center bg-[color-mix(in_srgb,_#fff_78%,_transparent)] text-accent-dark text-[0.9rem] font-bold rounded-[8px]" role="status">Updating minimum holding periods…</div>{/if}</div>
    <p class="chart-explanation holding-period-explanation mt-0 mb-4.5 text-[#5b7066] text-[0.8rem] leading-[1.45] mx-0">
  ...
  {:else if controller.benchmarkStatus.kind === 'success'}
    <p class="chart-explanation holding-period-explanation mt-0 mb-4.5 text-[#5b7066] text-[0.8rem] leading-[1.45] mx-0">Current-rate holding periods are unavailable because comparable recent CSH2 or overnight-rate data is missing.</p>
  {/if}

  {#if holdingPeriods && currentRateEvolution}
  <section class="panel chart-panel holding-period-chart-panel [background:var(--surface)] mx-0 my-4.5 p-5.5 border border-solid [border-color:var(--line)] rounded-[8px] [@media(width<=760px)]:p-5 [@media(760px<width<=1050px)]:p-4.5" aria-labelledby="holding-period-chart-heading">
    <div class="section-title flex items-center justify-between pb-5 gap-5 [@media(width<=760px)]:items-start [@media(width<=760px)]:flex-col [@media(width<=760px)]:gap-3"><div><p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Account comparison</p><h3 id="holding-period-chart-heading">CSH2 versus best savings account</h3></div></div>
    {#if accountRateIsEntered && accountRateIsValid}
      <div class="chart-update-container relative"><p class="chart-key holding-evolution-key flex flex-wrap mt-2.5 mb-1 text-[#5b7066] text-[0.8rem] mx-0 gap-4"><span class="chart-key-csh2">Net CSH2 advantage</span><span class="chart-key-account">Best savings account</span></p>
      <p class="chart-explanation holding-evolution-explanation mt-2.5 mb-3 text-[#5b7066] text-[0.8rem] leading-[1.45] mx-0">Assuming today’s rates stay constant, the line shows how far net CSH2 is ahead of or behind the best available savings account after buy and sell TOB and the selected tax. Arrows mark break-even and Match €STR.</p>
      <HoldingPeriodEvolutionChart points={currentRateEvolution.points} markers={evolutionMarkers} maximumDays={currentRateEvolution.maximumProjectionDays} valuationDate={holdingPeriods.valuationDate} />{#if controller.benchmarkStatus.kind === 'loading'}<div class="chart-update-overlay absolute z-2 grid place-items-center bg-[color-mix(in_srgb,_#fff_78%,_transparent)] text-accent-dark text-[0.9rem] font-bold rounded-[8px]" role="status">Updating account comparison…</div>{/if}</div>
    {:else}
      <p class="chart-empty grid min-h-72.5 place-items-center text-[#5b7066] text-[0.9rem] text-center m-0">Enter valid best-available savings rates to compare them with the current CSH2 projection.</p>
    {/if}
  </section>
  {/if}
</section>
