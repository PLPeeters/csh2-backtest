...
{#if holdingPeriods}
  <dialog class="methodology-dialog w-[min(680px,_-32px_+_100vw)] max-h-[min(760px,_-32px_+_100vh)] overflow-x-hidden overflow-y-hidden bg-white text-[#1d2925] [box-shadow:rgba(23,_36,_31,_0.28)_0px_24px_70px] p-0 border border-solid border-[#bdcbc3] rounded-[12px]" bind:this={methodologyDialog} aria-labelledby="methodology-title" aria-describedby="methodology-intro" onclick={(event) => { if (event.target === methodologyDialog) closeMethodology(); }} oncancel={(event) => { event.preventDefault(); closeMethodology(); }} onclose={unlockPageScroll}>
    <div class="methodology-dialog-header flex items-start justify-between grow-0 shrink-0 basis-auto [border-bottom-width:1px] [border-bottom-style:solid] [border-bottom-color:#e1e7e3] p-6.5 gap-5 [@media(width<=760px)]:p-5"><div><p class="eyebrow mb-1.5 text-muted text-[0.65rem] font-bold tracking-[0.09em] uppercase">Current-rate estimate</p><h3 id="methodology-title">How we estimate today’s CSH2 return</h3></div><button type="button" class="methodology-close w-8.5 min-h-8.5 grow-0 shrink-0 [flex-basis:34px] bg-white text-accent-dark text-[1.35rem] font-normal p-0 border-[#cbd7d0]" aria-label="Close methodology" onclick={closeMethodology}>×</button></div>
    <div class="methodology-dialog-content min-h-0 overflow-y-auto overscroll-contain pt-0 pb-6.5 px-6.5 [@media(width<=760px)]:pb-5 [@media(width<=760px)]:px-5">
      ...
      <ol class="methodology-steps grid pl-5.5 mx-0 my-5 gap-[13px]">
      ...
      {/if}
      <p class="methodology-caveat mt-3.5 mb-0 text-[#687970] text-[0.76rem] leading-[1.45] mx-0">This is an estimate, not a guaranteed return. The holding-time ranges assume today’s rates remain unchanged.</p>
    </div>
  </dialog>
{/if}
